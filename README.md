...
| 容器 | 内容 |
|------|------|
| `mux-db` | PostgreSQL 15 Alpine，复用持久化数据卷 |
| `mux-app` | FastAPI + Nginx + Vue 静态文件 |

需要 **Docker** 和 Node.js/npm；Node.js 只用于生成前端静态文件，不进入最终镜像。
...
`deploy.sh` 会在替换容器前把数据库完整导出到宿主机 `backups/`。数据库继续使用
`physics-lab-log_postgres_prod_data`，升级时不要执行 `docker compose down -v`。
...
docker exec mux-db sh -c 'PGPASSWORD="$POSTGRES_PASSWORD" pg_dumpall -U "$POSTGRES_USER"' > backups/manual.sql
```
...
|------|--------|------|
| `APP_PORT` | `18080` | 对外端口 |
| `DB_USER` | `mux_prod_admin` | 数据库角色；兼容已有数据卷 |
| `DB_PASSWORD` | 自动生成 | 数据库密码 |
| `SECRET_KEY` | 自动生成 | JWT 签名密钥 |
| `DB_NAME` | `mux_lab_logs` | 数据库名 |
| `DB_VOLUME_NAME` | `physics-lab-log_postgres_prod_data` | PostgreSQL 数据卷名 |
